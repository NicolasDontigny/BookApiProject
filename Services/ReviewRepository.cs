...
namespace BookApiProject.Services
{
    public class ReviewRepository : IReviewRepository
    {
        ...
        // Delete all Reviews from A Reviewer
        public bool DeleteReviews(List<Review> reviewsToDelete)
        {
            _reviewContext.RemoveRange(reviewsToDelete);
            return Save();
        }

        public Book GetBookOfAReview(int reviewId)
        {
            return _reviewContext.Reviews.Where(r => r.Id == reviewId).Select(r => r.Book).FirstOrDefault();
        }
